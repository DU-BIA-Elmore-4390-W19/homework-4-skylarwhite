...
results <- data_frame(ntree = seq(25, 500, by = 1),
                      mtry = rep(NA, length(ntree)),
                      RMSE = rep(NA, length(ntree)))

for (i in seq(25,500,50)){
...
results[i] <- rf_boston_cv$results$RMSE^2}

p <- ggplot(data = results, aes(x = ntrees, y = mse, col = as.factor(mttry)))
p + geom_point() +
  geom_line() +
...
boston_test_df <- boston_test_df %>%
  mutate(y_hat_gbm = test_preds,
         sq_err_gbm = (y_hat_gbm - medv)^2)
mean(boston_test_df$sq_err_gbm)
```

2. Fit a multiple regression model to the training data and report the 
estimated test MSE
```{r}
lm_boston <- lm(medv ~ ., data = training)
```
```{r}
test_preds <- predict(lm_boston, newdata = testing)
boston_test_df <- boston_test_df %>%
  mutate(y_hat_gbm = test_preds,
         sq_err_gbm = (y_hat_gbm - medv)^2)
mean(boston_test_df$sq_err_gbm)
